...
export default class TestQuizChallenge extends React.Component {
    ...
    fetchAPI = async () => {
        var dataArrayQuiz = [];
        for (let index = 0; index < this.state.dataArrayQuizChallenge[0].length; index++) {
            await axios.get("http://10.0.2.2:3000/quiz/" + this.state.dataArrayQuizChallenge[0][index].question_id).then(
                (response) => {
                    console.log(response.data);
    ...
    fetchApiChallenge = async () => {
        ...
        if (this.state.reading_id != null) {
            await axios.get("http://10.0.2.2:3000/quizInContent/" + this.state.reading_id).then(
                (response) => {
